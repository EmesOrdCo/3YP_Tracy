...
This chapter builds a longitudinal, rules-aware simulation of the Formula Student acceleration sprint so subsystem choices can be compared on a common quantitative basis before hardware is fixed.
...
owned elsewhere in this report. The model ranks design levers, rejects configurations that
breach FS-EV~2.2 or FS-D~5.3.1 before hardware commitment, predicts the
final agreed vehicle, and apportions margin across chapters.
...
This gives a design-side lever to
retain front normal force without lowering $h_{CG}$ or lengthening
$L$.
In this two-axle longitudinal model, $\text{AS}$ and $k_\text{AS}$ are a reduced-order surrogate for how strongly the rear anti-squat geometry in Section~\ref{sec:antisquat_dynamic_toe} may redistribute axle loads around the rigid-body transfer in Eq.~\ref{eq:load_transfer}; they are tuned for sprint time and wheelie margin rather than interpreted as numerically equal to the suspension chapter's geometric 100\% anti-squat line.
The factor $0.2$ caps $k_\text{AS}$ at $0.2$ when $\text{AS}=1$, keeping the effect conservative and numerically benign while remaining monotone in $\text{AS}$ for optimisation.
...
\noindent
Longitudinal Pacejka parameters for this project were triangulated against archived team fits and tyre-modelling threads on the \emph{FSAE} engineering forum~\cite{fsae_engineering_forum}; the Magic Formula implementation below expresses the resulting friction--slip characteristic at each driven wheel.
...
\end{equation}
where $I_\text{max}$ is the inverter AC current ceiling (\SI{285}{\ampere} here); the FS-EV~2.2 limit appears as $P_\text{max}/V_{\mathrm{bus}}(t)$, which \textbf{grows} as the bus sags because the same outlet power is supported at higher DC current at lower voltage until $I_\text{max}$ binds.
...
For this scalar estimate, $v^\star$ is evaluated at nominal $V_{\mathrm{bus}}=\SI{600}{\volt}$ together with the inverter torque ceiling $T_m^{\mathrm{peak}}=K_t I_{\max}$; a fully time-resolved bus would shift the crossover slightly.
Every acceleration event above this
...
Pack sizing and constant-power discharge, including series resistance, are developed in Section~\ref{subsec:supercapacitor_analysis}; selected key expressions are repeated below.
The longitudinal code does not instantiate a parallel Li-Po electrochemical branch.
\begin{gather}
...
The representative discharge trace falls to about \SI{369}{\volt} at the end of the \SI{4}{\second}, \SI{80}{\kilo\watt} template used there, whereas the powertrain assumes a minimum operating window on the order of \SI{350}{\volt} for inverter/motor operation (Figure~\ref{fig:motor_operation}).
That leaves usable margin at the bottom of the sag, so a falling DC link is acceptable: the inverter supplies torque by regulating motor current across the allowed voltage span, with $P_{\mathrm{elec}}\approx V_{\mathrm{bus}}(t)\,I_{\mathrm{dc}}$ implying higher DC-link current as $V_{\mathrm{bus}}(t)$ drops until FS-EV~2.2 or $I_\text{max}$ limits bind.
We identify that sagging trajectory with $V_{\mathrm{bus}}(t)$ in Eqs.~\ref{eq:powertrain_basic}--\ref{eq:power_limit}; further electrical transients are noted in Section~\ref{sec:discussion}.
...
Figure~\ref{fig:architecture} gives an overview of the package.
...
without re-running command-line scripts.
...
Peak simulated acceleration stays
...
Wheelbase & $L$ & 1.573\si{\metre} & Chassis layout (CAD) \\
CG longitudinal & $L_{CG}$ & 1.092\si{\metre} & Chassis layout (CAD) \\
CG height & $h_{CG}$ & 0.325\si{\metre} & Chassis layout (CAD) \\
...
(\SIrange{0.2}{0.4}{\second} omitted from the timestepping model).
Section~\ref{sec:conclusions} relates the Monte Carlo central estimate,
reaction-time bracketing extremes, and the FS-D~5.3.2 manual-mode
points readout used for competition scoring.
...
\noindent This summary restates the numerics without repeating Section~\ref{sec:discussion}.
...
The numerical search in Section~\ref{sec:optimisation} reduces the cold-design prediction from \SI{3.78}{\second} to \SI{3.67}{\second} under FS-EV~2.2, FS-D~5.3.1, and positive front load; wet grip scaling preserves the same compliance (Section~\ref{sec:drywet}).
...
Under FS-D~5.3.2 with $P_\mathrm{max}=75$ and a notional $T_\text{fastest}=\SI{4.0}{\second}$, the acceleration scoring readout uses the Monte Carlo mean (\SI{4.03}{\second}; Section~\ref{sec:monte_carlo}) plus driver-reaction extremes $+\SI{0.2}{\second}$ and $+\SI{0.4}{\second}$, giving on-event times \SI{4.23}{\second} and \SI{4.43}{\second}; on a linear bracket between $T_\text{fastest}$ and $T_\mathrm{max}=1.5\,T_\text{fastest}=\SI{6.0}{\second}$ (the usual FS~2025 manual-mode shape), those times correspond to about \num{66.8}/75 and \num{59.7}/75 respectively.

\subsection{Further work}
\label{sec:further_work}

Future development should focus on improving the physical fidelity of the model and increasing correlation with real-world vehicle behaviour.
...
Additional stochastic modelling of track contamination, tyre temperature evolution, and driver reaction variability would also improve robustness predictions for real competition conditions.
