...
\addbibresource{../YuzeJiang/references.bib}
...
\input{MichaelZiyangXing/fs_wheel_assembly_section}
\compactbib
\end{refsection}

\begin{refsection}
\graphicspath{{../YuzeJiang/Engineering/figures/}{../YuzeJiang/Engineering/}}
...
Therefore, the design approach focuses on maximising torque delivery while optimising system mass, resulting in a balanced optimisation of acceleration performance.
\subsection{Problem Definition}

Working from preliminary CAD simulations (Section 1.7.2), we decided although minimising vehicle mass as a whole does not necessarily optimise our performance (as discussed in Section 1.3), it is best to minimise the motor mass specifically to be compatible with the integrated motor mount design (proposed in Section 1.5) while maintaining sufficient power supply.

Additionally, the supercapacitor-based energy storage system introduces a varying supply voltage during discharge, which must be considered in the powertrain and control system design, while traction-related effects, such as slip, are addressed within the control strategy whilst the system must comply with safety requirements defined by the Formula Student rules. The shutdown circuit (SDC) provides fail-safe protection through independent hardware, operating separately from the controller to ensure safe system shutdown in fault conditions.

Other factors, such as cost and component lead time (the time between ordering and receiving the component), must also be considered as secondary factors in this acceleration-focused design.
...
Overall, the system is designed to transfer electrical energy from the accumulator to mechanical energy at the driveshaft while maintaining controllability and safety. A CAD drawing is shown in Figure~\ref{fig:CAD} to help visualization.

\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{CAD_Drawing.png}
    \caption{CAD Drawing for Powertrain Components (from Toby McConnell and Ziyang Xing)}
    \label{fig:CAD}
\end{figure}
...
The powertrain architecture adopts a centralised single-motor configuration, as determined by the overall vehicle design (based on analysis from Table~\ref{tab:pugh_drive_architecture}).

This configuration is selected to minimise drivetrain complexity and system mass, both of which directly benefit acceleration performance. While a multi-motor configuration would enable torque vectoring, however, it offers limited advantage in the 75,m straight-line acceleration event and introduces significant additional mass, outweighing any potential performance gain.

\subsection{Motor Requirements}

The primary requirement for the motor is to deliver high torque at low speeds, maximising tractive force during the acceleration event. And it must operate within the \SI{80}{\kilo\watt} power limit and \SI{500}{\ampere} current constraint imposed by FSUK rules\cite{fs-ev-rules2026}.
...
Even under high ambient temperatures, a temperature rise of only \SI{1.1}{\degreeCelsius} would keep the motor well below its typical continuous operating temperature of approximately \SI{50}{\degreeCelsius}, indicating negligible thermal risk during operation. This suggests that the motor is effectively over-designed in terms of cooling for this application. Therefore, future development could explore simplified cooling configurations in collaboration with the manufacturer, potentially reducing system mass and complexity. 
...
BAMOCAR 400/400 & 400 & 12 & 285 & 75 & 8.5 & 5000 \\
BAMOCAR 700/250 & 700 & 12 & 178 & 85 & 8.5 & 5000 \\
%need check for bamocar price
BAMOCAR 700/400 & 700 & 12 & 285 & 135 & 8.5 & 5000 \\
...
Based on the evaluation, the BAMOCAR PG-D3-700/400 achieves the highest overall score and is therefore selected as the inverter for the powertrain system, primarily due to its wide voltage operating range, which provides the greatest flexibility for the supercapacitor-based energy storage system, providing the most suitable solution for this application.
...
If an inconsistency is detected, the torque request is immediately set to zero to ensure safe operation. In addition, a brake override strategy is implemented, where motor torque is reduced to zero when braking is detected.
...
Based on the evaluation, the STM32 platform achieves the highest overall score and is therefore selected as the controller for the powertrain system, due to its balanced performance in cost, weight, processing capability, communication support, and reliability.
...
The SDC is implemented as a series-connected circuit of safety devices, including monitoring systems and interlock components. All elements must remain in a closed state for normal operation, and any triggered fault will open the circuit and deactivate the tractive system. This series configuration ensures that a fault in any individual component will activate the entire circuit, resulting in immediate shutdown by Accumulator Isolation Relays(AIRs).
...
Key elements include the Accumulator Management System (AMS), Insulation Monitoring Device (IMD), Brake System Plausibility Device (BSPD), and shutdown buttons. Among these, the Accumulator Isolation Relays (AIRs) act as the final switching elements of the shutdown circuit. When the SDC is closed, the AIR coils are energised, allowing the high-voltage system to remain connected.

%Air Pic maybe

When any fault condition is detected and the SDC is opened, the AIR coils are de-energised, immediately disconnecting the tractive system. This ensures that all fault conditions, regardless of their origin, result in a direct and reliable shutdown of the high-voltage system.

\section{Drivetrain Design}

\subsection{Design Overview}

The drivetrain system is responsible for transmitting torque from the motor to the driven wheels. It consists of a custom output shaft, driving and driven sprockets, and a chain. A chain drive configuration is selected due to its low mass, high efficiency, and ease of adjustment, making it well-suited for the short-duration acceleration application. All component selections prioritise lightweight design and compatibility with the single-run operating profile.

%maybe sprocket size? (ask Michael)

\subsection{Component Design and Selection}

The chain ratio of 5.5:1, as optimised from vehicle dynamics simulation detailed in Section 5.5.1, is achieved using an 16-tooth driving sprocket and 88-tooth driven sprocket with an ISO 606 12B-1 chain (pitch 19.05 mm). Chain tension under peak motor torque of 234 N·m was calculated as 4.82 kN, yielding a safe load of 28.94 kN based on a safety factor of 6 per ISO 606 requirements\cite{iso606}.

Material selection is conducted using an Ashby plot of strength against density, with strength thresholds derived from stress analysis:
\begin{equation}
    \sigma_y > 765\ \text{MPa (shaft)}, \quad \sigma_y > 5.67\ \text{MPa (sprocket)}, \quad \sigma_y > 32.5\ \text{MPa (chain)}
\end{equation}
GFRP is selected for the output shaft as it meets the strength requirement at significantly lower density than metallic alternatives. Nylon 6 is selected for both the chain and sprocket, offering adequate strength, low density, and lower cost than CFRP. The drivetrain scatter shield is constructed from 3\,mm aluminium alloy 6061-T6, which is lighter than the 2\,mm steel alternative permitted under FSUK rule\cite{fs-ev-rules2026}.
...
However, system-level considerations in Table~\ref{tab:pugh_brake_wheel} show that incorporating a differential enables a centralised braking configuration. Replacing two wheel-mounted braking systems with a single centralised braking system reduces the overall system mass, despite the addition of the differential. As the requirements specified, the use of inboard brakes reduces mechanical complexity and would be beneficial if it can achieve the estimated weight reduction, so this section is tasked to find the option with minimal mass.

\subsection{Differential Selection}

Four candidate units were evaluated against the criteria of torque capacity, weight, cost, and LSD type, as summarised in Table~\ref{tab:differential_selection}. A minimum torque capacity of 1158\,N$\cdot$m is required. Weight is assigned the highest weighting as it directly influences acceleration performance. The Drexler FSAE LSD achieves the highest overall score, primarily due to its low mass, and is therefore selected.
...
\renewcommand{\arraystretch}{1}
\setlength{\tabcolsep}{3pt}
\caption{Merit Matrix for Differential Selection}
\label{tab:differential_selection}
\small
Scoring scale: 0 = poor, 1 = acceptable, 2 = good, 3 = excellent.
Weighted score $= \sum (\text{score} \times \text{weighting})$.
\makebox[\textwidth][c]{
\begin{tabularx}{0.95\textwidth}{p{4 cm} *{4}{>{\centering\arraybackslash}X} >{\centering\arraybackslash}X}
\toprule
Differential & Torque Capacity & Weight & Cost & \textbf{Score} \\
\midrule
Weighting & Pass/Fail & 2 & 1 & -- \\
\midrule
\textbf{Drexler FSAE LSD\cite{Drexler_FSAE_LSD_data_sheet}} & Pass & 1 & 2 & \textbf{3} \\
Quaife ATB LSD\cite{Quaife_ATB_LSD} & Pass & 0 & 1 & 1 \\
Kaaz LSD\cite{Kaaz_LSD} & Fail & - & - & 0 \\
OS Giken LSD\cite{OSGiken_LSD} & Fail & - & - & 0 \\
\bottomrule
\end{tabularx}
}
\end{table}
...
\begin{table}[H]
\centering
\caption{Comparison of Drivetrain Configurations (Extended from Table~\ref{tab:bom_mass_wheel})}
\label{tab:drivetrain_comparison}
\begin{tabular}{lcccc}
\toprule
Configuration & Component & Quantity & Weight per Unit (kg) & Total Weight (kg) \\
\midrule
\multirow{5}{*}{Without Differential}
...
& Total & & & \textbf{10}\\
\midrule
\multirow{5}{*}{With Differential} 
& Braking System & 1 & 1.8 & 1.8 \\
& Upright & 2 & 0.8 & 1.6 \\
& Differential & 1 & 2.5 & 2.5 \\
& Axle & 2 halves & 1.3 & 2.6 \\
& Total & & & \textbf{8.5} \\
\bottomrule
\end{tabular}
\end{table}

The results indicate that the configuration with a differential achieves a lower total system mass. By selecting the Drexler differential and adopting a centralised braking configuration, the overall drivetrain and braking assembly mass is reduced from 10\,kg to 8.5\,kg, achieving a total mass saving of 1.5\,kg. Therefore, despite the additional differential component, the final configuration provides a more mass-efficient solution for the vehicle.
...
The inverter selection prioritises voltage range to accommodate the variable output of the supercapacitor system, resulting in the selection of the BAMOCAR PG-D3-700/400.A structured control system is developed to ensure stable operation, incorporating signal processing, plausibility checks, and constraint handling. Safety is ensured through a shutdown circuit designed in accordance with FSUK requirements.In addition, system-level optimisation is applied to the mechanical design. A lightweight drivetrain is implemented with a custom output shaft and material selection based on strength-to-weight considerations.The inclusion of a differential enables a centralised braking configuration, reducing overall system mass.
...
This chapter presents the accumulator design for the FS vehicle. The design choices are motivated by the same objectives to deliver short-term energy and high power working with our CAD model \ref{sec:chassis_architecture} and simulations (Chapter 10) to optmise mass and packaging volume, whilst also compiling with the specific FS electrical safety requirements.
...
To compare energy storage technologies, the following criteria were defined: (1) power output, (2) total energy capacity, and (3) a combined optimisation objective covering mass, volume, cost, and procurement risk. From the simulations (in Chapter 5), it was determined that the optimal mass for the battery for both traction and centre of mass distribution involves minimising the mass whilst keeping it above 20kg, as too low of a mass would reduce traction below optimal for peak performance. Since all full solutions considered are above this limit, the mass can simply be minimised.
...
This chapter details how we developed our bespoke, rules compliant simulation of the Formula Student acceleration event. This simulation has been used to inform subsystem design choices before hardware is fixed as seen by the preceeding chapters and also models the performance of the final car.
The Formula Student Acceleration Event (FS-D~5) is subject to two key rules that inform our simulation; FS-EV~2.2 which caps electrical power drawn from the
accumulator at \SI{80}{\kilo\watt}, and FS-D~5.3.1 disqualifies runs
over \SI{25}{\second}. 

A competitive configuration must be
...
Table~\ref{tab:pacejka}. They are derived from the Avon FSAE~\cite{fsae_engineering_forum} tyre
...
\phantomsection
\label{sec:thermal}
\addcontentsline{toc}{subsubsection}{Thermal grip window}
...
Wheelbase & $L$ & 1.573\si{\metre} & Chassis (Table~\ref{tab:layout_summary}) \\
CG longitudinal & $L_{CG}$ & 1.092\si{\metre} & Chassis (Table~\ref{tab:layout_summary}) \\
CG height & $h_{CG}$ & 0.325\si{\metre} & Chassis (Table~\ref{tab:layout_summary}) \\
...
Under FS-D~5.3.2 with  a notional $T_\text{fastest}=\SI{4.0}{\second}$ and a $T_\text{max}=\SI{6.0}{\second}$, the acceleration scoring using the Monte Carlo mean (\SI{4.03}{\second}; Section~\ref{sec:monte_carlo}) plus driver-reaction extremes $+\SI{0.2}{\second}$ and $+\SI{0.4}{\second}$,those times correspond to about 66.8 and 59.7 points out of a maximum 75.
...
\chapterauthor{Market \& Competition Analysis}{Toby McConnell}
\label{sec:market_and_competition}

\section{Introduction and Executive Summary}

Chapters 1 to 4 established the vehicle’s structural base and design elements (chassis, suspension, powertrain, and accumulator design), while Chapter 5 integrated these systems through simulation to validate its Monte Carlo 4.02-second sprint capability. Chapters 6 onwards pivots from technical engineering to commercial strategy.
...
Our service-based model allows clients to access advanced testing capabilities without the capital expenditure or maintenance burdens of in-house facilities. This value proposition is structured around four primary benefits:
...
\item \textbf{Scenario Flexibility} -- Modular configuration with different shells allowing clients to simulate different braking conditions and edge cases without the overhead of large facility changes.
...
\label{sec:ch6-gov-market-segmentation}
...
\label{sec:ch6-gov-commercial-challenges}
...
\subsection{Market Segmentation}
\label{sec:ch6-av-market-segmentation}
...
\label{sec:ch6-strategic-positioning}
...
\begin{itemize}
    ...
    \item \textbf{Regulatory infrastructure:} The UK hosts CAM Testbed UK, a government-backed national network of autonomous vehicle test facilities, alongside Zenzic and CCAV. Embedding the platform within this network provides a defined go-to-market mechanism specific to the UK (Section \ref{sec:cam_testbed_uk}).

    \item \textbf{Legislative timing:} The Automated Vehicles Act 2024 shifts legal liability onto AV developers and creates near-term UK-specific validation obligations, generating demand for physical testing evidence (Section \ref{sec:automated_vehicles_act_2024}).

    \item \textbf{Grant funding alignment:} Aligning with the UK government's £150 million CAM Pathfinder Programme provides a route to state-sponsored funding for commercial market readiness (Section \ref{sec:zenzic_roadmap}).
...
These user personas reflect the specific behaviors, motivations, pain points and decision-making behaviours of the people most likely to evaluate and procure the platform, grounding the targeting strategy in commercial reality. 
The stakeholder acceptance personas, testbed operators and liability assessors, who control access to the market are developed in Section \ref{sec:paul_personas}. Taken together, Kelly and Daniel illustrate that a successful sale requires the platform to satisfy two parallel tests: technical credibility for the engineer specifying the solution, and commercial justification for the procurement manager approving it: transparent, trustworthy data outputs are the common requirement across both.
...
The nascent nature of the market introduces a degree of uncertainty in demand, technology evolution, and regulatory direction. Demand for physical AV validation testing is closely linked to investment trends in automated driving technologies. Furthemore, a reduction in R\&D spending or a slowdown in AV deployment could reduce demand for specialised testing platforms. This risk is particularly relevant given the cyclical nature of investment in emerging technologies.
...
\noindent\textbf{Strategic through-line:} UK autonomous vehicle developers need repeatable physical edge-case validation without owning and maintaining target fleets. Embedding the AEB Platform as a Testing-as-a-Service (TaaS) platform within UK CAM testbeds creates value for AV developers, testbed operators, insurers, regulators, and the enterprise itself, but only if three conditions are met: testbed acceptance, credible safety evidence, and sufficient utilisation to justify the asset-heavy service model.
...
\label{sec:cam_testbed_uk}
\label{sec:ch7-cam-testbed}
...
\label{sec:sprint}
\label{sec:ch7-sprint-advantage}
This TaaS strategy is enabled by the mechanical profile of the adapted Formula Student (FS) architecture. The AEB Platform is designed for short, high-power manoeuvres rather than long duty cycles (Figure~\ref{tab:optim_result}).
...
Commercial viability relies on high utilisation and minimal track downtime. Because the AEB Platform will be subjected to physical impacts during edge-case validation, downtime control is a primary constraint.
...
\noindent\textbf{Implication:} TaaS converts the AEB Platform's short-duty-cycle sprint capability from a hardware limitation into a service advantage, because customers buy validated panic-manoeuvre evidence rather than vehicle ownership.
...
\label{sec:automated_vehicles_act_2024}
\label{sec:ch7-ethics-liability}
...
% ---------------------------------------------
\input{MichaelZiyangXing/eem_final}
% --- END INLINE: MichaelZiyangXing/eem_design_requirements_prototype_planning_compact.tex ---
\compactbib
\end{refsection}

\begin{refsection}
\graphicspath{{../YuzeJiang/Business/figures/}{../YuzeJiang/Business/}}
% --- BEGIN INLINE: Yuze Jiang/Business/EEM.tex ---
\chapterauthor{Supply Chain Design}{Yuze Jiang}

\section{Introduction}

This part of the report examines the supply chain design of a Formula Student vehicle used for autonomous emergency braking (AEB) testing.
...
\begin{figure}[h]
    ...
    \caption{Operating strategy positioning of the system within the responsiveness--uncertainty spectrum and Fisher variety--volume framework. The red markers indicate the operational characteristics of the vehicle testing system.}
...
To support this, a simplified operational Monte Carlo simulation is introduced using the failure probabilities and lead times obtained from the FMEA\cite{montecarlo}. Each simulation run represents a typical operational quarter consisting of 75 collision events, estimated from the financial and market growth analysis conducted by Harry. Collision events include both minor and major impacts occurring during testing activities. Component failures during each quarter are then estimated using a binomial distribution based on the corresponding failure probabilities obtained from the FMEA. And to reduce stochastic variation and improve the stability of the results, the simulation is repeated over 1000 Monte Carlo iterations.

\[
N_{\text{failure}} \sim \text{Binomial}(75,\; P_{\text{failure} \mid \text{collision}})
\]
...
As illustrated in Figure~\ref{fig:riskclassification}, components are classified according to two key dimensions: expected spare parts consumption and supplier lead time. Expected spare parts consumption reflects the inventory burden associated with each component, while lead time represents the level of supply responsiveness required for recovery.
...
Based on this, components are classified according to two key factors: failure likelihood and lead time, which together determine how frequently disruptions occur and how long the system is affected when they do. A conceptual classification is first introduced in Figure~\ref{fig:riskclassification}, defining four categories based on these dimensions.

The framework is then evaluated under collision operating conditions, as shown in Figure~\ref{fig:collision}. Under these conditions, several driveline components shift towards regions requiring higher inventory levels and more responsive supply chain strategies.
...
\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{Risk_classification.png}
    \caption{Component Risk Classification under Collision Conditions}
    \label{fig:collision}
\end{figure}
...
Pre-test Setup & System checks and preparation & $\sim$420 s & Non-value-added \\
...
\begin{itemize}
    \item Chassis: Toby McConnell (Chapter 1)
    \item Wheels: Ziyang Xing (Chapter 2)    
    \item Powertrain: Yuze Jiang (Chapter 3)
    \item Battery: Paul Vogelburg (Chapter 4)
\end{itemize}
...
\noindent Using the market, strategy, supply chain and prototyping analysis presented in the previous four chapters, this final chapter models and justifies our five-year financial plan using a
...
\begin{tabularx}{\textwidth}{@{}rr>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X|r@{}}
\hline
\textbf{Y} &
\textbf{Sold Days} &
\textbf{Blended Day rate (\pounds/d)} &
\textbf{Variable Cost (\pounds/d)} &
